...
try:
    ...
    if use_binance_live and not WEBSOCKETS_AVAILABLE:
        ...
        logger.warning("Live Binance Spot desactivado: %s", live_fallback_reason)
    ...
    if use_binance_live:
        ...
        if needs_seed:
            ...
            if seed_df is None or seed_df.empty:
                ...
                logger.warning("Live Binance Spot seed fallido: %s", live_fallback_reason)
                use_binance_live = False
            else:
        ...
        if use_binance_live:
            ...
            if datos.empty:
                ...
                if retry_df is not None and not retry_df.empty:
                    ...
                    live_fallback_reason = ws_err or retry_err or "No se recibieron velas live desde Binance Spot."
                    logger.warning("Live Binance Spot sin datos, fallback activado: %s", live_fallback_reason)
                    use_binance_live = False
    if not use_binance_live:
        stop_prev = st.session_state.get("binance_stop")
        if stop_prev:
        ...
        if live_mode_requested:
            st.sidebar.warning("Live Binance Spot no disponible en este servidor. Usando datos alternos (TwelveData/yfinance).")
...
if (
    ...
    status_label = "LIVE Binance Spot" if effective_live else "Fallback datos"
    meta_label = (
